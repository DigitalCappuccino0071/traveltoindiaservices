...
export default function StepOne() {
  ...
  // Use effect to handle redirects to ensure they happen after state updates
  // but make sure not to redirect after form submission which is handled by usePost
  useEffect(() => {
    // We're only redirecting for already existing form data
    // NOT for data that was just submitted (which usePost handles)
    const hasJustSubmitted = postMutation.isSuccess;

    if (!hasJustSubmitted && formId) {
      if (formData?.step1Data?.paid) {
        router.push('/visa/thankyou');
      } else if (formData?.step1Data) {
        router.push('/visa/step-one/update');
      }
    }
  }, [formId, formData?.step1Data, router, postMutation.isSuccess]);
  ...
  if (formId && (formData?.step1Data?.paid || formData?.step1Data)) {
    return <LoadingSkeleton />;
  }
